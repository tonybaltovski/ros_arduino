...
void raw_imu_callback(const ros_arduino_msgs::RawImuConstPtr& raw_msg){
  ...
  mz = raw_msg->raw_magnetic_field.z;
	...
	if(mx>mxh){mxh=mx; update=true;}
	else if(mx<mxl){mxl=mx; update=true;}
	if(my>myh){myh=my; update=true;}
	else if(my<myl){myl=my; update=true;}
	if(mz>mzh){mzh=mz; update=true;}
	else if(mz<mzl){mzl=mz; update=true;}
...
int main(int argc, char **argv){
  ...
  ros::Subscriber raw_imu_sub = n.subscribe("raw_imu", 10, raw_imu_callback);
